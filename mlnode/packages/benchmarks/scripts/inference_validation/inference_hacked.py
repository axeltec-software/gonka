...
from validation.utils_hacked import _extract_prefill_logprobs, inference_prefill  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument("--config-file", type=Path, default=None, help="Optional json config file to override CLI args.")
    #parser.add_argument("--no-token-str", action="store_true", default=False, help="Omit token_str field from inference_result.results entries.")
    ...
    if args.config_file:
        ...
        for k, v in config_data.items():
            if k == "request_params" and isinstance(v, dict):
                for rk, rv in v.items():
                    if rk == "max_tokens":
                        request_params.max_tokens = int(rv)
    ...
    def _work(prompt: str, lang: str) -> tuple:
        def _call():
            return inference_prefill(model_info, request_params, prompt)
