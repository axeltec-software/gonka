...
class VLLMRequestWrapperBase(VLLMRequestWrapper):
    @staticmethod
    ...
    @classmethod
    def _extract_enforced_tokens(cls, resp) -> EnforcedTokens:
        return cls._EnforcedTokens_from_content(resp["choices"][0]["logprobs"]["content"])
    ...
    def _extract_logprobs(cls, resp) -> Result:
        ...
        for position in logprobs:
            res = PositionResult(
                token=cls._extract_logprob_token(position["token"]),
                logprobs={cls._extract_logprob_token(logprob["token"]): logprob["logprob"] for logprob in position["top_logprobs"]}
            )
            results.append(res)
    ...
    @classmethod
    def _EnforcedTokens_from_content(cls, content: List[Dict[str, Any]]) -> "EnforcedTokens":
        tokens = []
        for position in content:
            token = cls._extract_logprob_token(position["token"])
            top_tokens = [cls._extract_logprob_token(x["token"]) for x in position["top_logprobs"]]
            tokens.append(EnforcedToken(token=token, top_tokens=top_tokens))
        return EnforcedTokens(tokens=tokens)


class VLLMRequestWrapper_AppendEnforcedTokensInVLLM(VLLMRequestWrapperBase):
    def inference(self,
        ...
        payload = {
            ...
            "messages": self._prepare_messages(prompt.string),
    ...
    def validation_response_to_logprobs_val(resp: Dict[str, Any], prompt_len: int):
        return resp["prompt_logprobs"][prompt_len:]


class VLLMRequestWrapper_AppendEnforcedTokensInPayload(VLLMRequestWrapperBase):
    def inference(self,
        model_info: ModelInfo,
        request_params: RequestParams,
        prompt: Prompt,
    ...
    @staticmethod
    def validation_response_to_logprobs_val(resp: Dict[str, Any], prompt_len: int):
        return resp["choices"][0]["prompt_logprobs"][prompt_len:]

###############################################################################

def _extract_prompt_logprobs(logprobs_val) -> Result:
    val_data = []
    token_ids = []
...
def generate_and_validate(
    ...
    try:
        req_wrapper = experiment_request.req_wrapper
        prompt = Prompt.from_string(experiment_request.prompt)

        inference_resp = req_wrapper.inference(
        ...
        prompt.set_tokens(inference_resp["prompt_token_ids"])
        ...
        validation_resp = req_wrapper.validation(
        ...
        logprobs_val = req_wrapper.validation_response_to_logprobs_val(validation_resp, inference_prompt_len)
        ...
        validation_tok_ids = [
            list(el.keys())[0]
            for el in req_wrapper.validation_response_to_logprobs_val(validation_resp, inference_prompt_len)
        ]
